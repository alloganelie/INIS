"""Confidence-signal LLM task per INIS spec §22.3 (signaux de confiance)."""
...
class ConfidenceSignalTask:
    """Evaluate confidence signals for a claim via the router."""

    task_type = "confidence_signal"
    ...
    async def run(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
        """Execute the confidence-signal task and return the result as a dict.

        Args:
            prompt: Prompt built by ``confidence_signal_prompt``.

"""Planning LLM task per INIS spec §22.3 (planifier les étapes)."""
...
class PlanningTask:
    """Generate an execution plan skeleton via the router."""
    ...
    async def run(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
        """Execute the planning task and return the result as a dict.

        Args:
            prompt: Planning prompt built by ``planning_prompt``.
